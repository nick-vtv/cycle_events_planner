...
from routes.views import RouteAddView, RouteDetailView, RouteUpdateView, RouteListView, MyRoutesListView

urlpatterns = [
    path('add/', RouteAddView.as_view(), name='add-route'),
    path('all/', RouteListView.as_view(), name='all-routes'),
    path('my-routes/', MyRoutesListView.as_view(), name='my-routes'),
            path('<int:pk>/', include([
